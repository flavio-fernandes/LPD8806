/*
Raspberry-pi library to control LPD8806-based RGB LED Strips
...
void LPD8806::bitBangLatchSignal() const {
...
void LPD8806::updateLength(uint16_t n) {
  ...
  if (n > 0 && (pixels = (uint8_t *) malloc(dataBytes))) { // Alloc new data
    numLEDs = n;
...
void LPD8806::show(void) {
  uint8_t  *ptr = pixels;
  uint16_t i    = (largestChangedLed + 1) * BYTES_PER_LED;
  uint8_t p, bit;
...
uint32_t LPD8806::Color(byte r, byte g, byte b) const {
...
void LPD8806::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
  if (n < numLEDs) { // Arrays are 0-indexed, thus NOT '<='
    uint8_t *p = &pixels[n * BYTES_PER_LED];
...
uint32_t LPD8806::getPixelColor(uint16_t n) const {
  if(n < numLEDs) {
    uint16_t ofs = n * BYTES_PER_LED;
